...
export class ExpenseFbProvider {
  ...
  getItemsByStatus(status: string): Observable<any[]> {
    return this.db.list('/expenseItems/', ref =>
      ref.orderByChild('status').equalTo(status)).snapshotChanges().
      pipe(
        map(changes =>
          changes.map(c => ({
            key:
  ...
  searchItems(val: string): Expense[] {
    ...
    return this.expenseList.filter(item => item.merchant.toLowerCase().includes(val) || item.category.toLowerCase().includes(val) || item.notes && item.notes.toLowerCase().includes(val));
  }
  addItem(item) {
